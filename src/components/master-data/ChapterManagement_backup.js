...
const ChapterManagement = () => {
  ...
  const fetchTopicsBySubject = async (subjectId, courseTypeId) => {
  ...
  const fetchChaptersData = async () => {
    try {
      ...
      if (filters.moduleId || filters.topicId || filters.subjectId || filters.classId || filters.examId || filters.courseId || filters.courseTypeId) {
        ...
        // Load all chapters if no filters
  ...
  }, [selectedCourseType, fetchCourses]);
  ...
  useEffect(() => {
    if (selectedCourse && selectedCourseType) {
      if (isAcademicCourseType(selectedCourseType)) {
        ...
        setFilteredClasses([]); // Clear classes for competitive courses
  ...
  // Effect for subject change - fetch topics by linkage (with de-dup)
  useEffect(() => {
    const courseTypeIdNum = selectedCourseType ? parseInt(selectedCourseType) : null;
    const relationshipIdNum = selectedSubject ? parseInt(selectedSubject) : null;
    if (!courseTypeIdNum || !relationshipIdNum) {
      lastTopicKeyRef.current = '';
      setFilteredTopics([]);
      return;
    }
    const key = `${courseTypeIdNum}|${relationshipIdNum}`;
    if (lastTopicKeyRef.current === key && filteredTopics.length > 0) {
      return;
    }
  ...
  // Effect for topic change - fetch modules by topic
  useEffect(() => {
    if (selectedTopic) {
  ...
  // Effect for module change - fetch chapters by module
  useEffect(() => {
    if (selectedModule) {
      fetchChaptersByModule(selectedModule, showActiveOnly).then((chaptersData) => {
        // Chapters are already set in useMasterData, we just need to trigger the fetch
      });
    }
  ...
  }, [formData.courseTypeId, fetchCourses]);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const submitData = {
        ...
        uploadedFileObjects: uploadedFileObjects // Include file objects for multipart upload
      };
  ...
  const handleEdit = (chapter) => {
    setFormData({
      ...
      courseTypeId: chapter.topic?.subject?.courseTypeId || '',
      courseId: chapter.topic?.subject?.courseId || '',
      classId: chapter.topic?.subject?.classId || '',
      examId: chapter.topic?.subject?.examId || '',
      subjectId: chapter.topic?.subject?.subjectId || '',
    ...
    <div className="master-data-component">
      <div className="component-header">
        <div className="header-info">
          ...
          <p>Manage chapters with hierarchical filtering, linking to modules, and multimedia content</p>
        </div>
      ...
      <div className="data-section">
        ...
        {(loading || chaptersLoading) ? (
          ...
          <div className="data-grid">
            {(combinedChapters.length > 0 ? combinedChapters : chapters).map((chapter) => (
              <ChapterListCard
                key={chapter.id}
                chapter={chapter}
