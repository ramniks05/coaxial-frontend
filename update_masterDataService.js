...
// Read the current masterDataService.js file
const filePath = path.join(__dirname, 'src', 'services', 'masterDataService.js');
...
// Replace createChapter function
const createChapterRegex = /export const createChapter = async \(token, chapterData\) => \{[\s\S]*?\n\};/;
content = content.replace(createChapterRegex, newCreateChapter);

// Replace updateChapter function
const updateChapterRegex = /export const updateChapter = async \(token, id, chapterData\) => \{[\s\S]*?\n\};/;
content = content.replace(updateChapterRegex, newUpdateChapter);

// Write the updated content back to the file
fs.writeFileSync(filePath, content, 'utf8');

console.log('✅ Updated masterDataService.js with multipart API support');
